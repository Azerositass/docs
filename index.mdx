...
If you need to load the widget script asynchronously (with `async` or `defer` attributes), use the queue pattern to avoid race conditions:

```html
<!-- Step 1: Initialize the queue BEFORE the script loads -->
<script>
...
<!-- Step 2: Load the script asynchronously -->
<script src="https://wearo.io/widget.js" async></script>
```

**Why use this?**
- The queue ensures your `init()` call is processed even if it happens before the script finishes loading
- Once the script loads, it automatically processes all queued calls

**When to use this:**
- If you use `async` or `defer` on the script tag
- If your script tag is in the `<head>` and you call `init()` in the `<body>`

**Standard (Synchronous) Loading:**  
If you place both the `<script src="...">` and the `TryOnWidget.init()` call at the end of your `<body>` without `async`/`defer`, you don't need the queue pattern - the standard method works fine.
...
| `productSelector` | string | No | `img[data-product-image], img.product-image, .product-image img, .product__image img, .product__media img, .woocommerce-product-gallery__image img` | CSS selector to find product images |
...
#### Widget button appears below the image or misplaced

Since the widget respects your site's layout and injects the button as a sibling element, this can happen if your image container is larger than the image itself.

**Solutions:**
1. **CSS Fix:** Add `width: fit-content;` to your product image container.
2. **Manual Positioning:** You can target the button container `#tryon-trigger-host` in your CSS to adjust its position (e.g., `margin-bottom: 20px;`).

---

## Support

- **Email:** support@wearo.io
